...
function tryLogin(req) {
  ...
  var ret= {'success':true,'sessionid':123};  
...
function checkSession(req) {
  ...
  if(req.session===123) return {'alive':true}; 
  var ret={'alive':false};
...
function getDate(req) {
 ...
 var starton3=new Date(req.date);
  ...
  var ret = { "date": req.date, "timecards": [ {"id":1, "starton": starton1, "endon": endon1, "chargenumber": "REF", "reason": "", "comments": "Coding" },{"id":2, "starton": starton3, "endon": endon3, "chargenumber": "MCTIMS", "reason": "", "comments": "Coding" }, {"id":3, "starton":starton2, "endon": endon2, "chargenumber": "FUZION", "reason": "", "comments": "DB Design" }, {"id":4, "starton":starton4, "endon": endon4, "chargenumber": "REF", "reason": "", "comments": "DB Design" }] };
...
function getTimecard(req) {
  ...
  endon1.setHours(12);
  endon1.setMinutes(0);
  var ret={"timecard":{"id":req.id, "starton": starton1, "endon": endon1, "chargenumber": 1, "reason": "", "comments": "Coding" }, "chargenumbers":[{"id":1,"name":"REF"},{"id":2,"name":"MCTIMS"},{"id":3,"name":"Fuzion"}],"reasons":[{"id":1,"name":"Forgot"},{"id":2,"name":"Charge number not available"},{"id":3,"name":"Out of office"}]};
...
function saveTimecard(req) {
  ...
  if(!req.reason && starton.getTime()<today.getTime()) {
    ret.success=false;
    ret.messages.push("You must supply a reason for a previous day's entry.");  
  }
...
function newTimecard(req) {
  ...
  if(new Date(req.date)) 
    ret= {"guid":5,"chargenumbers":[{"id":1,"name":"REF"},{"id":2,"name":"MCTIMS"},{"id":3,"name":"Fuzion"}],"reasons":[{"id":1,"name":"Forgot"},{"id":2,"name":"Charge number not available"},{"id":3,"name":"Out of office"}]};
